...
* [ 安装EayunOS虚拟化管理中心宿主机节点 ](#安装EayunOS虚拟化管理中心宿主机节点)
  * [ 启动安装 ](#启动安装)
  * [ 安装EayunOS虚拟化管理中心宿主机 ](#安装EayunOS虚拟化管理中心宿主机)
  * [ 配置EayunOS虚拟化管理中心宿主机 ](#配置EayunOS虚拟化管理中心宿主机)
    * [ 登录Hypervisor ](#登录Hypervisor)
    * [ 选择Hypervisor键盘 ](#登录Hypervisor)
    * [ 观察Hypervisor的状态 ](#观察Hypervisor的状态)
    * [ 配置Hypervisor网络 ](#观察Hypervisor的状态)
   * [ Hypervisor安全配置 ](#Hypervisor安全配置)
    * [ 配置Hypervisor简单网络管理协议（SNMP） ](#配置Hypervisor简单网络管理协议（SNMP）)
    * [ 配置Hypervisor通用信息模型 ](#配置Hypervisor通用信息模型)
    * [ 配置Hypervisor日志记录 ](#配置Hypervisor日志记录)
...
* [ Hosted Engine的部署 ](#Hosted Engine的部署)
  ...
  * [ 添加第一台高可用节点 ](#添加第一台高可用节点)
  * [ EayunOS虚拟化管理中心的初始化配置 ](#EayunOS虚拟化管理中心的初始化配置)
  * [ 访问管理员门户 ](#访问管理员门户)

* [ 添加EayunOS虚拟化管理中心宿主机 ](#添加EayunOS虚拟化管理中心宿主机)
  * [ 作为高可用节点 ](#作为高可用节点)
  * [ 作为普通节点 ](#作为普通节点)
    * [ 通过EayunOS虚拟化宿主机注册 ](#通过EayunOS虚拟化宿主机注册)
    * [ 通过管理门户添加 ](#通过管理门户添加)
...
1. EayunOS管理员账户的密码。
1. Hosted Engine虚拟机所使用的存储类型，可以是NFS或iSCSI。应在安装前应配置好所选类型的存储服务。

## 安装EayunOS虚拟化管理中心宿主机节点
### 启动安装
1. 插入EayunOS安装盘
  1. 启动系统，并确定系统从安装光盘启动。
...
* 摘要
  ...
  由于使用Hosted Egnine的架构，EayunOS虚拟化管理中心里将存在两种主机：提供给Engine虚拟机运行的高可用节点和提供给起他虚拟机运行的普通节点。当然，高可用节点也可以运行起他虚拟机，但为了使Engine虚拟机能够更好地工作，我们不推荐使用高可用节点运行普通主机。

### 作为高可用节点

* 摘要
  * Hosted Engine部署成功后，默认已经有了第一台高可用节点。
  * 当需要支持EayunOS管理端的高可用时，需要添加更多的高可用节点。
...
1. 交互内容完成，主机作为高可用节点到EayunOS虚拟化管理中心，状态变化为Installing -> Up。

* 结果

  高可用节点添加成功，HostedEngine虚拟机可以运行在这台高可用节点上。
...
1. 批准主机
  ...
  1. 点菜单中的【批准】按键。出现【编辑和批准主机】对话框。你可以使用该对话框修改该主机的名字，核对它的SSH指纹，也可以修改这台主机将要加入的数据中心和集群，还可以给支持电源管理卡的主机配置电源管理。
