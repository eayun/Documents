# Trend Reports: Single Virtual Machine Resources(Hour Of Day)

**Single Virtual Machine Resources(Hour Of Day)** 报表显示了在一个指定时间范围内，一台虚拟机的各种资源使用率指标，并按照一天 24 个小时(0:00 ~ 23:00)对它们进行分组。这些指标包括了 CPU 使用率，内存使用率，磁盘使用率和网络使用率。运行该报表需要提供以下参数：
